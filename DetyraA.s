...
.text
    ...
    la $a0, vektori                 # vektorin e dergojme si parameter
...
populloVektorin:
    ...
    li $v0, 4
    la $a0, jepNrAnt              # printohet fjalia jepNrAnt
    syscall
...
#-------------------------------------------------------------------------------
    ...
    ruaj: 
        ...
        syscall
        sw $v0, 0($s0)            # po e rujme ne vektor vleren e dhene prej perdoruesit             |MBUSHJA E VEKTORIT
        addi $s0, $s0, 4          # e inkrementojme per 4 adresen ku do ta shkruajme numrin e radhes ne vektor
        addi $t1, $t1, 1          # i++
...
#-------------------------------------------------------------------------------
    ...
    loop: 
        ...
        mul $t3, $t0, 4
        add $t5, $s0, $t3         # adresen baze te vektorit po e inkrementojme per vleren e $t4(p * 4), 
                                  # sepse nje numer i plote ne vektor eshte 4bytes, prandaj edhe indeksi duhet te jete shumefish i 4
        ...
        mul $t4, $t0, 4           # $t3 e kemi deklaru ma nalt si 4, se na duhet per me nxjerr elemente nga array[indeksi shumefish i 4]
        ...
        mul $t3, $t2, 4
        add $t5, $s0, $t3         # loc si shumefish i 4, passi po i qasemi vektorit
...
unazavlerave:
    ...
    loop2:
        ...
        mul $t3, $t7, 4           # k * 4
        add $t5, $s2, $t3         # inkrementohet adresa baze e vektorit dhe kjo velere ($t5) paraqet 
                                  # adresen e sakte per elementin qe duam t'i qasemi
        lw $t6, 0($t5)            # a[k]
        ...
        blt $t1, $t6, goto        # if min>a[k]
            ...
            move $t2, $t7         # loc = k
        goto:
            addi $t7, $t7, 1      # po edhe nese nuk gjindet su bo nomi, kalojme ne iterimin tjt
            j loop2
